...
export function InvoicePaymentView({ bookingId }: InvoicePaymentViewProps) {
  ...
  const initInvoiceData = React.useCallback(async (isBackground = false) => {
    try {
      ...
      let inv = await invoiceService.getBookingInvoice(bookingId);
      if (!inv && b && ["BILL_GENERATED", "PAYMENT_PENDING", "PAYMENT_RECEIVED", "BOOKING_COMPLETED"].includes(b.status)) {
        inv = await invoiceService.createInvoice({
          bookingId: b.id,
