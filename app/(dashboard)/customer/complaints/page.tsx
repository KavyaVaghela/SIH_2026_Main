...
export default function CustomerComplaintsPage() {
  ...
  const [complaints, setComplaints] = React.useState<GrievanceCase[]>([]);
  ...
  const fetchComplaints = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/complaints?role=CUSTOMER&actorId=${customerId}`);
  ...
  const getStatusBadge = (status: GrievanceLifecycleStatus) => {
    switch (status) {
      ...
      case "ACTION_REQUIRED":
        return <Badge className="bg-purple-100 text-purple-800 border-purple-300 dark:bg-purple-950 dark:text-purple-300 font-bold text-xs">Clarification Requested</Badge>;
      case "RESOLVED":
    ...
    <div className="space-y-6 pb-12 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
            My Grievances & Disputes
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Track filed complaints, respond to federation mediation requests, and view official resolutions.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <Card className="p-10 text-center border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-2xl space-y-3">
          ...
          <h3 className="font-bold text-sm text-slate-900 dark:text-slate-100">You don&apos;t have any grievances.</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            Have an issue with a service or worker? You can register a formal grievance with cooperative conciliation.
          </p>
          <div className="flex items-center justify-center gap-2 pt-1">
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => router.push("/customer/guidance?q=grievance")}
          ...
          {filtered.map((item) => (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                ...
                <Button
                  size="sm"
                  onClick={() => router.push(`/customer/complaints/${item.id}`)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                ...
                <div>
                  <span className="text-[10px] text-slate-400 uppercase font-bold block">Against</span>
                  <span className="font-semibold text-slate-800 dark:text-slate-200">
                    {item.targetName || "Assigned Worker"}
                  </span>
                </div>

                <div>
                  <span className="text-[10px] text-slate-400 uppercase font-bold block">Filing Date</span>
              ...
              {item.status === "ACTION_REQUIRED" && (
                <div className="p-2.5 bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800 rounded-lg text-xs text-purple-900 dark:text-purple-200 flex items-center justify-between">
                  <span className="font-bold">Federation mediation officer requested your statement.</span>
                  <Button
                    size="sm"
                    onClick={() => router.push(`/customer/complaints/${item.id}`)}
