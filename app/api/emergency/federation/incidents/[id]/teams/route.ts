import { NextRequest, NextResponse } from "next/server";
import { getAuthenticatedUser } from "@/lib/emergency/auth-helper";
import { EmergencyIncidentRepository } from "@/lib/emergency/incident-store";
import { EmergencyTeamRepository } from "@/lib/emergency/team-store";

export async function GET(
  request: NextRequest,
  context: { params: Promise<{ id: string }> }
) {
  try {
    const authUser = await getAuthenticatedUser(request);
    if (!authUser) {
      return NextResponse.json(
        { error: "Unauthorized: Active authenticated session required." },
        { status: 401 }
      );
    }

    if (authUser.role !== "FEDERATION_ADMIN" && authUser.role !== "SUPER_ADMIN") {
      return NextResponse.json(
        { error: "Forbidden: Access restricted to Federation Administrators." },
        { status: 403 }
      );
    }

    const { id: incidentId } = await context.params;
    if (!incidentId) {
      return NextResponse.json({ error: "Incident ID is required." }, { status: 400 });
    }

    const incident = await EmergencyIncidentRepository.findById(incidentId);
    if (!incident) {
      return NextResponse.json({ error: "Emergency incident not found." }, { status: 404 });
    }

    if (authUser.role !== "SUPER_ADMIN" && incident.federation_id !== authUser.federationId) {
      return NextResponse.json(
        { error: "Forbidden: Incident belongs to another federation." },
        { status: 403 }
      );
    }

    const teams = await EmergencyTeamRepository.listTeamsForIncident(incidentId);

    return NextResponse.json({
      success: true,
      teams,
      count: teams.length,
    });
  } catch (err: unknown) {
    console.error("GET /api/emergency/federation/incidents/[id]/teams error:", err);
    return NextResponse.json(
      { error: (err as Error)?.message || "Internal server error" },
      { status: 500 }
    );
  }
}

export async function POST(
  request: NextRequest,
  context: { params: Promise<{ id: string }> }
) {
  try {
    const authUser = await getAuthenticatedUser(request);
    if (!authUser) {
      return NextResponse.json(
        { error: "Unauthorized: Active authenticated session required." },
        { status: 401 }
      );
    }

    if (authUser.role !== "FEDERATION_ADMIN" && authUser.role !== "SUPER_ADMIN") {
      return NextResponse.json(
        { error: "Forbidden: Access restricted to Federation Administrators." },
        { status: 403 }
      );
    }

    const { id: incidentId } = await context.params;
    if (!incidentId) {
      return NextResponse.json({ error: "Incident ID is required." }, { status: 400 });
    }

    const incident = await EmergencyIncidentRepository.findById(incidentId);
    if (!incident) {
      return NextResponse.json({ error: "Emergency incident not found." }, { status: 404 });
    }

    if (authUser.role !== "SUPER_ADMIN" && incident.federation_id !== authUser.federationId) {
      return NextResponse.json(
        { error: "Forbidden: Incident belongs to another federation." },
        { status: 403 }
      );
    }

    const body = await request.json();
    const {
      teamType = "SECONDARY",
      parentTeamId = null,
      requiredWorkerCount = 2,
      requiresTeamLead = false,
      teamLeadWorkerId = null,
      reason = "Additional response team formed to scale incident capacity",
    } = body;

    // Check duplicate worker if team lead specified
    if (teamLeadWorkerId) {
      const existingTeams = await EmergencyTeamRepository.listTeamsForIncident(incidentId);
      const isAlreadyAssigned = existingTeams.some((t) =>
        (t.members || []).some((m) => m.worker_id === teamLeadWorkerId && m.status !== "RELEASED" && m.status !== "NO_SHOW")
      );

      if (isAlreadyAssigned) {
        return NextResponse.json(
          { error: "Conflict: This worker is already assigned to an active team on this incident." },
          { status: 409 }
        );
      }
    }

    const newTeam = await EmergencyTeamRepository.createAdditionalTeam({
      incidentId,
      federationId: incident.federation_id || authUser.federationId || "",
      teamType,
      parentTeamId,
      requiredWorkerCount: Number(requiredWorkerCount) || 2,
      requiresTeamLead,
      teamLeadWorkerId,
      reason,
      actorId: authUser.id,
    });

    return NextResponse.json(
      {
        success: true,
        team: newTeam,
      },
      { status: 201 }
    );
  } catch (err: unknown) {
    console.error("POST /api/emergency/federation/incidents/[id]/teams error:", err);
    return NextResponse.json(
      { error: (err as Error)?.message || "Internal server error" },
      { status: 500 }
    );
  }
}
