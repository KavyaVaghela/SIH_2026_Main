...
export async function POST(
  ...
  try {
    ...
    if (authUser.role !== "FEDERATION_ADMIN" && authUser.role !== "SUPER_ADMIN") {
      return NextResponse.json(
        { error: "Forbidden: Only Federation Administrators or Super Admins can archive emergency incidents." },
        { status: 403 }
      );
    }
    ...
    if (authUser.role === "FEDERATION_ADMIN" && incident.federation_id) {
      ...
      if (userFedId && incident.federation_id !== userFedId) {
        return NextResponse.json(
          { error: "Forbidden: You may only archive incidents within your assigned federation." },
          { status: 403 }
        );
      }
    }

    const result = await EmergencyIncidentRepository.archiveIncident({
      incidentId,
    ...
    return NextResponse.json({
      success: true,
      message: "Emergency incident archived and removed from active list.",
      incident: result.record,
    });
  } catch (err: unknown) {
    console.error("POST /api/emergency/incidents/[id]/archive error:", err);
    return NextResponse.json(
